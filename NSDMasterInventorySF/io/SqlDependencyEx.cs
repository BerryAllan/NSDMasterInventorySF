...
namespace NSDMasterInventorySF.io
{
	public sealed class SqlDependencyEx : IDisposable
	{
		...
		public void Stop()
		{
			...
			{
				//Debug.WriteLine("null or cancel requested");
				return;
			}
		...
		private void NotificationLoop(object input)
		{
			try
			{
				while (true)
				{
					...
					//Debug.WriteLine(string.IsNullOrWhiteSpace(message) ? "no message" : message);
					Active = true;
			...
			{
				// ignored
				Debug.WriteLine("Notification Loop failed someway");
			}
			finally
			{
				Active = false;
